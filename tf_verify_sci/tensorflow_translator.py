            ...
            with tf.Session() as sess:
                ...
                for op in graph.get_operations():
                    ...
                    if op.type == "MatMul":
                        ...
                        filters, image_shape, strides, pad_top, pad_left, pad_bottom, pad_right = self.conv2d_resources(
                            op)
                        deeppoly_res = (filters, image_shape, strides, pad_top, pad_left, pad_bottom,
                                        pad_right) + in_out_info
                        deepzono_res = deeppoly_res
                        operation_resources.append({'deepzono': deepzono_res, 'deeppoly': deeppoly_res})
                    elif op.type == "MaxPool" or op.type == "AvgPool":
                        image_shape, window_size, strides, pad_top, pad_left, pad_bottom, pad_right = self.pool_resources(
                            op)
                        deeppoly_res = (image_shape, window_size, strides, pad_top, pad_left, pad_bottom,
                                        pad_right) + in_out_info
                        deepzono_res = deeppoly_res
                        operation_resources.append({'deepzono': deepzono_res, 'deeppoly': deeppoly_res})
