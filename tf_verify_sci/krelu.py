...
from relu_hull.krelu_methods import krelu_with_sci, krelu_with_sciplus
...
def sparse_heuristic_with_cutoff(length, lb, ub, K=3, s=-2):
    ...
    while len(vars_above_cutoff) > 0 and sparse_n >= K:
        ...
        if grouplen <= K:
            ...
            # print(grouplen, K, s, config.approx_k, config.epsilon, )
    ...
    kact_args.extend([var] for var in all_vars)

    print(f"Neurons Spliting Zero: {len(all_vars)} "
          f"Affter Cutoff: {n_vars_above_cutoff} "
          f"Group Number: {len(kact_args)}")
...
def encode_kactivation_cons(nn, man, element, offset, layerno, length, lbi, ubi, constraint_groups, need_pop, domain,
    ...
    with multiprocessing.Pool(config.numproc) as pool:
        ...
        kact_results = list(pool.starmap(make_kactivation_obj, zip(input_hrep_array, lb_array, ub_array,
    ...
    if config.debug:
        print(
            f'total k-activation time: {end - start:.3f}. Time for input: {end_input - start:.3f}. Time for k-activation constraints {end - end_input:.3f}.')
    if domain == 'refinezono':
