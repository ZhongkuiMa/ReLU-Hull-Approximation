...
class ERAN:
    ...
    def analyze_box(self, specLB, specUB, domain,
                    ...
                    timeout_lp=config.timeout_lp, timeout_milp=config.timeout_milp, use_milp=config.use_milp,
                    ...
                    uexpr_cst=None, uexpr_dim=None, expr_size=0, label=-1, prop=-1, testing=False,
                    spatial_constraints=None, terminate_on_failure=True):
        """
        ...
        """
        assert domain in ['deepzono', 'refinezono', 'deeppoly',
                          'refinepoly'], "domain isn't valid, must be 'deepzono' or 'deeppoly'"
        ...
        if domain == 'deepzono' or domain == 'refinezono':
            ...
            analyzer = Analyzer(execute_list, nn, domain, output_constraints, label, prop,
                                K=K, s=s, approx_k=approx_k,
                                use_default_heuristic=use_default_heuristic, testing=testing,
    ...
    def analyze_zonotope(self, zonotope, domain, timeout_lp, timeout_milp, use_default_heuristic,
        ...
        if domain == 'deepzono' or domain == 'refinezono':
            ...
            analyzer = Analyzer(execute_list, nn, domain, timeout_lp, timeout_milp, output_constraints,
                                use_default_heuristic, testing, prop=prop)
        elif domain == 'deeppoly' or domain == 'refinepoly':
            assert 0
