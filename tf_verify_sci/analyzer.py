...
class Analyzer:
    def __init__(self, ir_list, nn, domain, output_constraints, label, prop,
                 ...
                 complete=configparser.ConfigParser, use_milp=config.use_milp,
    ...
    def get_abstract0(self):
        ...
        for i in range(1, len(self.ir_list)):
            if type(self.ir_list[i]) in [DeeppolyReluNode, DeeppolySigmoidNode, DeeppolyTanhNode, DeepzonoRelu,
                                         DeepzonoSigmoid, DeepzonoTanh]:
                        ...
                        if label == adv_label:
                            ...
                            if (self.domain == 'refinepoly'):
                                ...
                                if self.complete:
                                    ...
                                    assert model.Status not in [3,
                                                                4], f"Model status is {model.Status}: Infeasible Model encountered"
                                    ...
                                    if model.Status == 6 or (model.Status == 2 and model.objval > 0):
                                        ...
                                        if model_partial_milp.Status in [2, 9, 11] and model_partial_milp.ObjBound > 0:
                                            ...
                                            print("Partial milp model was not successful status is",
                                                  model_partial_milp.Status)
