...
def prepare_model(model):
    """
    The constructor has produced a graph_def with the help of the functions graph_util.convert_variables_to_constants
    and graph_util.remove_training_nodes.
    translate() takes that graph_def, imports it, and translates it into two lists which then can be processed by an
    Optimzer object.

    Return
    ------
    (operation_types, operation_resources) : (list, list)
        A tuple with two lists, the first one has items of type str and the second one of type dict. In the first list
        the operation types are stored (like "Add", "MatMul", etc.).
        In the second list we store the resources (matrices, biases, etc.) for those operations. It is organised as
        follows: operation_resources[i][domain] has the resources related to
        operation_types[i] when analyzed with domain (domain is currently either 'deepzono' or 'deeppoly',
        as of 8/30/18)
    """
    shapes_map = {}
    constants_map = {}
    ...
    for initializer in model.graph.initializer:
        const = nchw_to_nhwc(numpy_helper.to_array(initializer))
        constants_map[initializer.name] = const
        shapes_map[initializer.name] = const.shape

    placeholder_names = []
    for input_node in model.graph.input:
        placeholder_names.append(input_node.name)
        if input_node.name not in shapes_map:
            shapes_map[input_node.name] = onnxshape_to_intlist(input_node.type.tensor_type.shape)
            input_nodes_map[input_node.name] = input_node

    for node in model.graph.node:
        output_nodes_map[node.output[0]] = node

        for input_node in node.input:
            input_nodes_map[input_node] = node

        if node.op_type == "Flatten":
            shapes_map[node.output[0]] = [1] + [np.prod(shapes_map[node.input[0]][1:]).tolist()]
        elif node.op_type == "Constant":
            const = node.attribute
            const = nchw_to_nhwc(numpy_helper.to_array(const[0].t)).copy()
            constants_map[node.output[0]] = const
            shapes_map[node.output[0]] = const.shape
            ...
            for attribute in node.attribute:
                if 'transA' == attribute.name:
                    ...
                    transB = attribute.i
            input_shape_A = ([1] if len(shapes_map[node.input[0]]) == 1 else []) + list(shapes_map[node.input[0]])
            ...
            shapes_map[node.output[0]] = [M, N]

        elif node.op_type in ["Add", "Sub", "Mul", "Div"]:
            shapes_map[node.output[0]] = shapes_map[node.input[0]]
            ...
            output_shape = []
            input_shape = shapes_map[node.input[0]]
            ...
            shapes_map[node.output[0]] = output_shape
        ...
        # Gather is for the moment solely for shapes
            ...
            if node.input[0] in shapes_map and node.input[1] in shapes_map:
                r = len(shapes_map[node.input[0]])
                q = len(shapes_map[node.input[1]])
                out_rank = q + r - 1
                if out_rank == 0:
                    shapes_map[node.output[0]] = shapes_map[node.input[1]]
                else:
                    output_shape = []
                    for i in range(out_rank):
                        if i < axis:
                            output_shape.append(shapes_map[node.input[0]][i])  # i < axis < r
                        elif i >= axis and i < axis + q:
                            output_shape.append(shapes_map[node.input[0]][i - axis])  # i - axis < q
                        else:
                            output_shape.append(shapes_map[node.input[0]][i - q + 1])  # i < out_rank < q + r - 1
                    shapes_map[node.output[0]] = output_shape
        elif node.op_type == "Shape":
            if node.input[0] in shapes_map:
                constants_map[node.output[0]] = shapes_map[node.input[0]]
                shapes_map[node.output[0]] = [len(shapes_map[node.input[0]])]
        ...
        # print("CASTING ", node.input[0], shape_map[node.input[0]], shape_map[node.output[0]])
            ...
            if node.input[1] in constants_map:
                ...
                if replace_index != -1:
                    constants_map[node.input[1]][replace_index] = np.prod(shapes_map[node.input[0]]) / total

                if len(constants_map[node.input[1]]) == 4:
                    shapes_map[node.output[0]] = [constants_map[node.input[1]][0], constants_map[node.input[1]][2],
                                                  constants_map[node.input[1]][3], constants_map[node.input[1]][1]]
                else:
                    shapes_map[node.output[0]] = constants_map[node.input[1]]

        elif node.op_type == "Unsqueeze":
            if node.input[0] in shapes_map:
                axis = node.attribute[0].ints
                output_shape = list(shapes_map[node.input[0]])
            ...
            all_constant = True
            n_dim = len(shapes_map[node.input[0]])
            if n_dim > 2:
                axis = nchw_to_nhwc_index(node.attribute[0].i)
            else:
                axis = node.attribute[0].i
            for input_node in node.input:
                if not input_node in constants_map:
                    all_constant = False
                    break
            if all_constant:
            ...
            for input_node in node.input:
                if not input_node in shapes_map:
                    all_shape_known = False
                    break
            assert all_shape_known, "Unknown shape for at least one node input!"
            new_axis_size = 0
            for input_node in node.input:
                new_axis_size += shapes_map[input_node][axis]
            shapes_map[node.output[0]] = [shapes_map[node.input[0]][i] if i != axis else new_axis_size for i in
                                          range(len(shapes_map[node.input[0]]))]
            if not all_constant:
            ...
            input_shape = list(shapes_map[node.input[0]])
            ...
            if node.input[1] in constants_map:
                ...
                result = np.zeros(shapes_map[node.output[0]]) + constants_map[node.input[0]]
                constants_map[node.output[0]] = result
        elif node.op_type == "Pad":
            input_shape = np.array(shapes_map[node.input[0]])
            for attribute in node.attribute:
            ...
            shapes_map[node.output[0]] = list(output_shape)
        else:
...
class ONNXTranslator:
    ...
    def __init__(self, model, is_gpupoly):
        """
        This constructor takes a reference to a ONNX Model and checks model, infers intermediate shapes and sets up maps
        from name to type and node or constant value
        graph_util.convert_variables_to_constants and graph_util.remove_training_nodes to cleanse the graph of any nodes
        that are linked to training. This leaves us with
        the nodes you need for inference.
        In the resulting graph there should only be tf.Operations left that have one of the following types [Const,
        MatMul, Add, BiasAdd, Conv2D, Reshape, MaxPool, AveragePool, Placeholder, Relu, Sigmoid, Tanh, LeakyRelu]
        If the input should be a Keras model we will ignore operations with type Pack, Shape, StridedSlice, and Prod
        such that the Flatten layer can be used.
        ...
        if issubclass(model.__class__, onnx.ModelProto):
            ...
            self.shape_map, self.constants_map, self.output_node_map, self.input_node_map, self.placeholdernames = \
                prepare_model(model)
        else:
    ...
    def translate(self):
        """
        The constructor has produced a graph_def with the help of the functions
        graph_util.convert_variables_to_constants and graph_util.remove_training_nodes.
        translate() takes that graph_def, imports it, and translates it into two lists which then can be processed by an
        Optimzer object.
        ...
        ------
        (operation_types, operation_resources) : (list, list)
            A tuple with two lists, the first one has items of type str and the second one of type dict. In the first
            list the operation types are stored (like "Add", "MatMul", etc.).
            In the second list we store the resources (matrices, biases, etc.) for those operations. It is organised as
            follows: operation_resources[i][domain] has the resources related to
            operation_types[i] when analyzed with domain (domain is currently either 'deepzono' or 'deeppoly', as of
            8/30/18)
        """
        ...
        in_out_placeholder = (
            [], placeholder.name, self.clean_shape(onnxshape_to_intlist(placeholder.type.tensor_type.shape)))
        operation_resources = [{'deepzono': in_out_placeholder, 'deeppoly': in_out_placeholder}]
        reshape_map = {}
        operations_to_be_ignored = ["Pack", "Shape", "StridedSlice", "Prod", "Unsqueeze", "Softmax", "Concat",
        ...
        for node_idx, node in enumerate(self.nodes):
            ...
            if (extract_norm and node_idx <= stop_norm_layer and len(operation_types) == 2
                    and node.op_type in ["Add", "Sub", "Mul", "Div"]
                    and node.output[0] not in self.constants_map):
            ...
            if node.op_type == "MatMul":
                ...
                filters, bias, image_shape, strides, pad_top, pad_left, pad_bottom, pad_right, kernel_shape = self.conv_resources(
                    node)
                if node.name in padding_merger_dict:
                    image_shape, in_out_info, pad_top, pad_left, pad_bottom, pad_right = self.merge_padding(node,
                                                                                                            padding_merger_dict,
                                                                                                            in_out_info,
                                                                                                            pad_top,
                                                                                                            pad_left,
                ...
                image_shape, kernel_shape, strides, padding, dilations, pad_top, pad_left, pad_bottom, pad_right, ceil_mode, storage_order = self.pool_resources(
                    node)
                if node.name in padding_merger_dict:
                    image_shape, in_out_info, pad_top, pad_left, pad_bottom, pad_right = self.merge_padding(node,
                                                                                                            padding_merger_dict,
    ...
    def reshape_adjust(self, element, matrix, is_right=False):
        if self.get_kind(element) in ['Reshape',
                                      'Flatten'] and not self.is_gpupoly:  # TODO check whether it should be triggered for Flatten layers to
