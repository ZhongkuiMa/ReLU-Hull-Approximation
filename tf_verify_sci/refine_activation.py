...
def refine_activation_with_solver_bounds(nn, self, man, element, nlb, nub, relu_groups, timeout_lp, timeout_milp,
                                         use_default_heuristic, domain, K=3, s=-2, use_milp=False, approx=True):
    """
    ...
    if nn.activation_counter == 0:
        ...
        if 0 < sum((affine_layers >= second_FC).__and__(
                predecessor_index >= affine_layers)) <= config.n_milp_refine:  # predecessor_index >= second_FC :#and domain=="deepzono" and predecessor_index>second_FC:
        ...
        if config.refine_neurons == True and nn.layertypes[predecessor_index] == "FC":
            ...
            if config.debug:
                print(
                    f"Refinement of bounds time: {end - start:.3f}. MILP used: {use_milp_temp}. {len(indices)} bounds refined.")
        ...
        if domain == 'deepzono':
            encode_kactivation_cons(nn, man, element, offset, predecessor_index, length, lbi, ubi, relu_groups, False,
