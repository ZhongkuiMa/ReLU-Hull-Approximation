...
class Optimizer:
        ...
        while i < nbr_op:
            if self.operations[i] == "MatMul":
                ...
                if i < nbr_op - 1 and self.operations[i + 1] == "BiasAdd":
                    filters, image_shape, strides, pad_top, pad_left, pad_bottom, pad_right, c_input_names, _, _ = \
                    self.resources[i][domain]
                    bias, _, b_output_name, b_output_shape = self.resources[i + 1][domain]
                    i += 2
                else:
                    filters, image_shape, strides, pad_top, pad_left, pad_bottom, pad_right, c_input_names, b_output_name, b_output_shape = \
                    self.resources[i][domain]
                ...
                if domain == 'deepzono':
                    execute_list.append(
                        DeepzonoConvbias(image_shape, filters, bias, strides, pad_top, pad_left, pad_bottom, pad_right,
                                         c_input_names, b_output_name, b_output_shape))
                else:
                    execute_list.append(
                        DeeppolyConv2dNode(filters, strides, pad_top, pad_left, pad_bottom, pad_right, bias,
                                           image_shape, c_input_names, b_output_name, b_output_shape))
                nn.numlayer += 1
            elif self.operations[i] == "Conv":
                filters, bias, image_shape, strides, pad_top, pad_left, pad_bottom, pad_right, c_input_names, output_name, b_output_shape = \
                self.resources[i][domain]
                nn.numfilters.append(filters.shape[3])
                nn.filter_size.append([filters.shape[0], filters.shape[1]])
                nn.input_shape.append([image_shape[0], image_shape[1], image_shape[2]])
                ...
                if domain == 'deepzono':
                    execute_list.append(
                        DeepzonoConvbias(image_shape, filters, bias, strides, pad_top, pad_left, pad_bottom, pad_right,
                                         c_input_names, output_name, b_output_shape))
                else:
                    execute_list.append(
                        DeeppolyConv2dNode(filters, strides, pad_top, pad_left, pad_bottom, pad_right, bias,
                                           image_shape, c_input_names, output_name, b_output_shape))
                i += 1
            ...
            #     i += 1
                ...
                nn.numlayer += 1
                i += 1
            elif self.operations[i] == "MaxPool" or self.operations[i] == "AveragePool" or self.operations[
                i] == "AvgPool":
                image_shape, window_size, strides, pad_top, pad_left, pad_bottom, pad_right, input_names, output_name, output_shape = \
                self.resources[i][domain]
                ...
                if domain == 'deepzono':
                    execute_list.append(
                        DeepzonoPool(image_shape, window_size, strides, pad_top, pad_left, pad_bottom, pad_right,
                                     input_names, output_name, output_shape, is_maxpool))
                else:
                    execute_list.append(
                        DeeppolyPoolNode(image_shape, window_size, strides, pad_top, pad_left, pad_bottom, pad_right,
                                         input_names, output_name, output_shape, is_maxpool))
                i += 1
            elif self.operations[i] == "Pad":
                image_shape, pad_top, pad_left, pad_bottom, pad_right, input_names, output_name, output_shape = \
                self.resources[i][domain]
                nn.input_shape.append([image_shape[0], image_shape[1], image_shape[2]])
                nn.padding.append([pad_top, pad_left, pad_bottom, pad_right])
                nn.out_shapes.append(output_shape)
                ...
                if domain == 'deepzono':
                    ...
                    execute_list.append(
                        DeeppolyPaddingNode(pad_top, pad_left, pad_bottom, pad_right, image_shape, input_names,
    ...
    def get_deeppoly(self, nn, specLB, specUB, lexpr_weights, lexpr_cst, lexpr_dim, uexpr_weights, uexpr_cst, uexpr_dim,
                     expr_size, spatial_constraints=None):
        """
