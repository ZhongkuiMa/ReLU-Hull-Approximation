...
def refine_gpupoly_results(nn, network, num_gpu_layers, relu_layers, true_label, labels_to_be_verified,
                           ...
                           complete=config.complete, method=config.approx_k):
    is_successful = True
    relu_groups = []
    nlb = []
    nub = []
    ...
    for l in relu_layers:
        ...
        if config.refine_neurons == True:
            ...
            for i in range(length):
                if ((nlb[predecessor_index][i] < 0 and nub[predecessor_index][i] > 0) or (
                        nlb[predecessor_index][i] > 0)):
        ...
        for i_a in range((int)(np.ceil(A.shape[0] / max_eqn_per_call))):
            ...
            bounds_temp = network.evalAffineExpr(A_temp, layer=gpu_layer, back_substitute=network.FULL_BACKSUBSTITUTION,
                                                 dtype=np.double)
        ...
        with multiprocessing.Pool(config.numproc) as pool:
            ...
            kact_results = list(
                pool.starmap(make_kactivation_obj, zip(input_hrep_array, lb_array, ub_array,
                                                       [method] * len(input_hrep_array)
                                                       )))
        print(f"kact Used Time: {time.time() - start:.4f}s")
    ...
    for label in labels_to_be_verified:
        ...
        print("Constrs Number:", model.getAttr("NumConstrs"))
        # model.computeIIS()
        # model.write("model_refinegpupo.ilp")
        try:
            print(f"Model status: {model.Status}, "
                  f"Obj val/bound against label {label}: {model.objval:.4f}/{model.objbound:.4f}, "
                  f"Final solve time: {model.Runtime:.3f}")
        except:
            print(f"Model status: {model.Status}, "
                  f"Objval retrival failed, "
                  f"Final solve time: {model.Runtime:.3f}")
        ...
        if model.Status == 6 or (model.Status in [2, 11] and model.objval > 0):
            ...
            if not is_not_shown_unsafe:
                ...
                if model_partial_milp.Status == 3:
                    ...
                    model_partial_milp.optimize(milp_callback)
                try:
                    print(f"Partial MILP model status: {model_partial_milp.Status}, "
                          f"Obj val/bound against label {label}: {model_partial_milp.ObjVal:.4f}/{model_partial_milp.ObjBound:.4f}, "
                          f"Final solve time: {model_partial_milp.Runtime:.3f}")
                except:
                    print(
                        f"Partial MILP model status: {model_partial_milp.Status}, "
                        f"Obj bound retrival failed, "
                        f"Final solve time: {model_partial_milp.Runtime:.3f}")
